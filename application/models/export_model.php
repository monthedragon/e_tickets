...
class Export_model extends CI_Model {
    ...
    public function set_default_condition(){
        ...
        if(isset($post['callresult']) && !empty($post['callresult']))
            $this->db->where('contact_list.callresult',$post['callresult']);
					
				$has_sub_cr = array('CB','NI');
        if(isset($post['sub_callresult']) && !empty($post['sub_callresult']) && in_array($post['callresult'],$has_sub_cr))
            $this->db->where('contact_list.sub_callresult',$post['sub_callresult']);
					
        if(isset($post['ag_type']) && !empty($post['ag_type']) && $post['callresult'] == 'AG')
            $this->db->where('contact_list.ag_type',$post['ag_type']);
	...
	function get_remarks(){
		...
		$this->db->select('contact_list.id,trans_log.remarks,user_id')->from('contact_list')
